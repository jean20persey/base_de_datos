    ...
    if nombre and apellido and fecha_nac and telefono and correo and cargo and salario:
        try:
            with conexion.cursor() as cur:
                sql = '''INSERT INTO DatosEmpleados (NumCedula, Nombre, Apellido, FechaNac, Correo, Telefono)
                        VALUES (?, ?, ?, ?, ?, ?)'''
                cur.execute(sql, (0, nombre, apellido, fecha_nac, correo, telefono))
                conexion.commit()
...
def modificar():
    ...
    if seleccion:
        ...
        def guardar_modificaciones():
            ...
            grid.item(seleccion, values=(detalles_empleado['values'][0], nuevo_nombre, nuevo_apellido, nueva_fecha, nuevo_telefono, nuevo_correo, nuevo_cargo, nuevo_salario))
            ventana_modificar.destroy()

        ventana_modificar = tk.Toplevel()
        ventana_modificar.title("Modificar Empleado")

        nuevo_nombre_label = tk.Label(ventana_modificar, text="Nuevo Nombre: ")
        nuevo_nombre_label.grid(row=0, column=0)

        txt_nombre_mod = tk.Entry(ventana_modificar)
        txt_nombre_mod.grid(row=0, column=1)
        txt_nombre_mod.insert(0, detalles_empleado['values'][1])

        guardar_button = tk.Button(ventana_modificar, text="Guardar", command=guardar_modificaciones)
        guardar_button.grid(row=1, columnspan=2)
...
def eliminar():
    ...
    if seleccion:
        ...
        try:
            with conexion.cursor() as cur:
                sql = "DELETE FROM DatosEmpleados WHERE ID = ?"
...
def llenar_datos():
    ...
    try:
        with conexion.cursor() as cur:
            cur.execute("SELECT * FROM DatosEmpleados")
            datos = cur.fetchall()
            for row in datos:
                grid.insert("", END, text=row[0], values=(row[2], row[3], row[4], row[5], row[6], row[7], row[8]))
    except Exception as e:
...
def main():
    ...
    Bt_limpiar = tk.Button(frame1, text="Limpiar", command=limpiar_campos, bg="pink", fg="white")
    Bt_limpiar.place(x=5, y=80, width=80, height=30)

    Bt_modificar = tk.Button(frame1, text="Modificar", command=modificar, bg="pink", fg="white")
    Bt_modificar.place(x=5, y=125, width=80, height=30)

    Bt_eliminar = tk.Button(frame1, text="Eliminar", command=eliminar, bg="pink", fg="white")
